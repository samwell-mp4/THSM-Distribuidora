...
const CIDADE = {
  ...
  'conselheiro lafaiete': [-20.66, -43.78], 'ouro branco': [-20.52, -43.69],
  'mariana': [-20.38, -43.42], 'vicosa': [-20.75, -42.88], 'ponte nova': [-20.41, -42.91],
  ...
  'santa rita do sapucai': [-22.25, -45.70],
...
function makeIcon(color, size) {
  return L.divIcon({
    className: '',
    html: `<div style="background:${color};width:${size}px;height:${size}px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:${Math.round(size*0.42)}px;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.25);cursor:pointer"><i class="fa-solid fa-user"></i></div>`,
    iconSize: [size, size],
...
export default function MapView({ usuarios }) {
  ...
  useEffect(() => {
    ...
    for (const u of users) {
      ...
      const e = u.endereco || {}
      const coords = cityCoord(e.cidade, e.estado)
      if (!coords) continue

      items.push({ id, user: u, coords })
      const isSel = sel.has(id)
      const mk = L.marker(coords, { icon: makeIcon(isSel ? '#dc2626' : '#2563eb', isSel ? 30 : 26) })

      const nome = u.nome || u.pushName || '—'
      const addr = fullAddr(e)
      mk.bindPopup(`
    ...
    setPts(items)
    if (hasB) m.fitBounds(bounds, { padding: [40, 40], maxZoom: 12 })
  }, [users, sel])

  useEffect(() => {
    if (!map.current) return
    const g = rotaG.current
    g.clearLayers()
  ...
  const tracar = useCallback(async () => {
    const selPts = pts.filter(p => sel.has(p.id))
    ...
    <div className="mv">
      <div className="mv-top">
        <div>
          ...
          <div className="mv-sub">{users.length} usuários · {pts.length} cidades · {sel.size} selecionados</div>
        </div>
        <div className="mv-actions">
          {sel.size >= 2 && (
            <button className="mbtn mbtn-red" disabled={calc} onClick={tracar}>
              <i className={`fa-solid ${calc ? 'fa-spinner fa-spin' : 'fa-route'}`}></i> Traçar Rota ({sel.size})
            </button>
          )}
          {sel.size > 0 && (
      ...
      <div className="mv-list">
        ...
        <div className="mv-lb">
          ...
          {pts.length === 0 && <div className="mv-e">Nenhum usuário com endereço completo</div>}
        </div>
      </div>
    </div>
  )
